...
# 第5回「ニューケインジアン・モデルの新展開」のソースコードのRへの野良移植: 非線形モデル
#
...
dparam <- list(
	...
	sigma_R = 0.22/100, # ϵ_Rの標準偏差
	sigma_g = 0.71/100, # ϵ_gの標準偏差
	sigma_z = 0.31/100, # ϵ_zの標準偏差
...
calcY_star <- function(gnow){
	with(dparam, (1-nu)^(1/tau)*exp(g_bar)*exp(gnow))
}

calcC <- function(fc0){
	with(dparam, fc0^(-1/tau))
}

calcPi <- function(c, fp0){
	with(dparam, {
		invnu <- nu^-1
		a0 <- 0.5*phi*ss$pi^2/nu + (1-invnu) + c^tau/nu + fp0*c^tau
		a1 <- -phi*ss$pi*(1-invnu)
		a2 <- -phi*(1-0.5*invnu)
		(a1-sqrt(a1^2-4*a0*a2))/2/a2
		})
}

calcY <- function(c, pi, gnow){
	with(dparam, c/(1/exp(g_bar)/exp(gnow) - phi/2*(pi - ss$pi)^2))
}

calcR <- function(y, pi, R0, rnow, y_star){
	with(dparam, R0^rho_R*(ss$R*(pi/ss$pi)^psi_1*(y/y_star)^psi_2)^(1-rho_R)*exp(rnow))
}
...
r_solve <- with(dparam, {
	...
	policy_0 <- with(ss, { initPolicyF(c, pi, R, y, 0, 0, 0, 0) })
	...
	# ループ内で使う変数fcx, fpxの計算用関数
	calc_fcx <- function(c1, pi1, R, zp){
		# rnss = ass*piss/bet から、Juliaのソースコードにあったassを消した
		c1^(-tau)*R/(beta*ss$R/ss$pi*exp(zp))/pi1
	}

	calc_fpx <- function(y0, y1, c1, pi1){
		c1^(-tau)*y1*(pi1-ss$pi)*pi1/y0
	}
	...
	for(i in 1:maxit){
		...
		y0n = calcY(c0n, pi0n, gnow)
		R0n = calcR(y0n, pi0n, Rpast, rnow, y_star)

		# in the ZLB regime
		c0b <- calcC(fvec0[, "c0b"])
		pi0b <- calcPi(c0b, fvec0[, "pi0b"])
		y0b = calcY(c0b, pi0b, gnow)
		R0b = calcR(y0b, pi0b, Rpast, rnow, y_star)
		...
		for(iv in 1:nrow(fvec1)){
			...
			r_sapply <- sapply(1:ncol(shocks), function(iz){
				...
				y1b <- calcY(c1b, pi1b, gp[iv])
				R1b <- 1

				fcxb <- calc_fcx(c1b, pi1b, R0b[iv], zp[iv])
				fpxb <- calc_fpx(y0b[iv], y1b, c1b, pi1b)

				# then check if the ZLB is violated by rn1, and use coeffcb and coeffpb instead
				if (is.na(R1n) || R1n<1.0){
					fcxn <- fcxb
					fpxn <- fpxb
					R1n <- R1b
				}
				...
				r <- c(
					ghweights[iz]*beta*fcxn,
					ghweights[iz]*beta*phi*fpxn,
					ghweights[iz]*beta*fcxb,
					ghweights[iz]*beta*phi*fpxb
				)
