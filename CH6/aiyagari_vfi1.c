...
#define Nk 100 /* grid size for state and control */
...
/* C++と異なりC言語は関数の冒頭に変数宣言をまとめる必要がある */
	...
	double wage, phi, *gridk, minK, intK, *v, *tv, *kfun, *vtemp, cons, vpr, t1, err=0, diff, util, *mea0, *mea1, result;
	int *kfunG, *kfunG_old;
	int nor, noc, Nl, diff_int, err_int;
	 /* ループ用の変数群 */
	unsigned i, iter, kc, lc, lcc, kcc;
...
*/
		for(kc=0; Nk>kc; kc++){ /* kcが0からはじまるのに注意 */ 
			for(lc=0; Nl>lc; lc++){ /* lcが0からはじまるのに注意 */
				...
				for(kcc=0, kccmax=Nk-1; Nk>kcc; kcc++){
					cons = REAL(s)[lc]*wage + (1 + *REAL(r))*gridk[kc] - gridk[kcc];
					if(cons<=0){
						kccmax = kcc - 1;
						break;
					}
					...
					for(vpr=0, lcc=0; Nl>lcc; lcc++){
						vpr += REAL(prob)[lc + nor*lcc]*v[p(lcc, kcc)]; /* probは他と縦横サイズが異なる */
					}
		...
		for(kc=0; kc<Nk; kc++){
			for(lc=0; lc<Nl; lc++){
				...
				for(lcc=0;lcc<Nl;lcc++){
					mea1[p(lcc, kcc)] = mea1[p(lcc, kcc)] + REAL(prob)[lc + nor*lcc]*mea0[p(lc, kc)];
				}            
			}        
		}
